...
inline void jdip::macro_function::preparse(string val, error_handler *herr)
{
  ...
  for (pt i = 0; i < val.length(); ) // Iterate the string
  {
    ...
    if (val[i] == '#')
    {
      ...
      if (!is_letter(val[i])) { herr->error("Expected parameter name following '#' token; `" + val + "'["); continue; } //--DXG MAC BUILD HACK + ::toString(i) + "] is not a valid identifier character"); continue; }
      const size_t asi = i;
...
bool macro_function::parse(vector<string> &arg_list, char* &dest, char* &destend, token_t errtok, error_handler *herr) const
{
  if (arg_list.size() < args.size()) {
    if (arg_list.size() + 1 < args.size())
      return errtok.report_error(herr, "Too few arguments to macro function `" + name + "': provided "), false; //--DXG MAC BUILD HACK + ::toString(arg_list.size()) + ", requested " + ::toString(args.size())), false;
    arg_list.push_back("");
  }
