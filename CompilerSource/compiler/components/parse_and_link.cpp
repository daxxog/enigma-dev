...
int lang_CPP::compile_parseAndLink(EnigmaStruct *es,parsed_script *scripts[], vector<parsed_script*>& tlines, const std::set<std::string>& script_names)
{
  ...
  for (int i = 0; i < es->scriptCount; i++)
  {
    ...
    //--DXG MAC BUILD HACK edbg << "Parsed `" << es->scripts[i].name << "': " << scripts[i]->obj.locals.size() << " locals, " << scripts[i]->obj.globals.size() << " globals" << flushl;
  ...
  for (int i=0; i<es->timelineCount; i++) 
  {
    for (int j=0; j<es->timelines[i].momentCount; j++) 
    {
      ...
      //--DXG MAC BUILD HACK edbg << "Parsed `" << es->timelines[i].name <<", moment: " <<es->timelines[i].moments[j].stepNo << "': " << tlines.back()->obj.locals.size() << " locals, " << tlines.back()->obj.globals.size() << " globals" << flushl;
